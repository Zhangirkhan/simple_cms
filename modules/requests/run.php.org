<?php
		...
		if (isset($_POST['OSIpushToUCSend'])) {
			...
			$id = intval($_POST['id']);
			$sql = "UPDATE module_requests SET uc_id = ".$uc_id.", osi_accepted = 1 WHERE id = ".$id."";
		...
		if (isset($_SESSION['id'])) {
			...
			if($_SESSION['type'] == 1 ){
				...
				$result = db_query("SELECT * FROM module_requests WHERE ".$user_type_id." = ".$_SESSION['id']." AND request_status_id = 11 AND uc_accepted = 0 OR ".$user_type_id." = ".$_SESSION['id']." AND request_status_id = 1 AND uc_accepted = 0 ORDER BY time_create DESC");
			}else{
