...
function GetAction2ByTabIDAndSessionTypeID($tab, $sessionType, $id ,$sessionID, $from, $osi_accepted , $uc_id, $uc_accepted , $worker_uc_id, $worker_uc_accepted ) {
    ...
    if($tab == 1){
        if($sessionType == 3){ // Житель
            if($osi_accepted == 0){ // Заявка создана но ее еще не приняли
                $action = '<a href="#" class="btn btn-danger" data-toggle="tooltip" data-target="#delete" onclick="deleteRequest('.$id.','.$sessionID.','.$sessionType.');">Удалить</a> ';
            }else if($osi_accepted == 1)
        ...
        if($sessionType == 3){  // Житель
            $action = '<a href="#" class="btn btn-primary" data-toggle="tooltip" onclick="recycleRequest('.$id.', '.$sessionType.' );" >Дублировать</a> ';
        }
    }else if($tab == 3){
        ...
        if($sessionType == 3){  // Житель
            ...
            $action = ' <a href="#" class="btn btn-success" id="AddReviewButton" data-toggle="modal" data-target="#AddReview" data-id="'.$id.'" data-stuff="'.$sessionType.'" data-author="'.$sessionID.'" >Принять</a> 
    ...
    $action .= ' <a href="#" class="btn btn-gray" data-toggle="tooltip"  onclick="requestHistory('.$id.')">История</a>';
    return $action;
}
