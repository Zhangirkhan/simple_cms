...
if (isset($_SESSION['id'])) {
	...
	if (db_num_rows($result) > 0) {
		...
		while ($row = db_fetch_array($result)) {
			...
			$tpl->assign("TIME_CREATED", '<span class="badge badge-primary">'.date("d.m.Y", strtotime($row['time_start'])). '</span>');
			$tpl->assign("ACTION", $action);
			$tpl->parse("NOT_INSERTED_ROWS", "." . $moduleName . "row");
		}
	} else {
