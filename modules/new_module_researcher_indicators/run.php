<?php
	$moduleName = "new_module_researcher_indicators";
	...
	if (checkAccessPage($_PAGE['mod_rewrite'])) {
		...
		if (isset($_GET['item_id'])) { //Внутренная страница
			...
			$users_deparments = db_query("SELECT * FROM new_module_permisions WHERE new_user_id = ".$_SESSION['id']);
			...
			while ($department = db_fetch_array($users_deparments)) {
				...
				$departments_array .= ",'".$department['indicator_category_id']."'";
			}

			$users_indicators = db_query("SELECT * FROM new_module_indicators WHERE id IN (".$departments_array.")");
			if ( db_num_rows($users_indicators) > 0) {
				while ($row2 = db_fetch_array($users_indicators)) {
					$table=$row2['indicator_table_name'];
					...
					// 	}
						// echo $full_result;
					$result = db_query($first_result);
					if ( db_num_rows($result) > 0) {
						while ($row = db_fetch_array($result)) {
							$tpl->assign("GENERAL_ID", $row['id']);
							$tpl->assign("NAME", $row2['name']);
							$action = '<div class="btn-group" role="group">'; // start group
							$action .= '<a class="btn btn-success btn-sm text-white" data-toggle="tooltip" onclick="addNewData(\''.$row['indicator_table_name'].'\')" data-original-title="'.getval("STR_APPROVE").'"><i class="fa fa-check"></i> '.getval("STR_APPROVE").'</a>';
							$action .= '<a class="btn btn-danger btn-sm text-white" data-toggle="tooltip" onclick="addNewData(\''.$row['indicator_table_name'].'\')" data-original-title="'.getval("STR_CANCEL").'"><i class="fa fa-times"></i> '.getval("STR_CANCEL").'</a>';
							...
							$category = db_get_data("SELECT name FROM new_module_indicator_categories WHERE id = " . $row2['id'], "name");
							$tpl->assign("CATEGORY",  $category);

							$user = db_get_data("SELECT fio FROM new_module_users WHERE id = " . $row['new_user_id'], "fio");
							...
							$tpl->assign("BALL",  $ball);

							$user_file = "./files/".$row['file_url'];
							$tpl->assign("FILE", '<a href="'.$user_file.'" > Посмотреть файл</a>');

							if($row['status']  == 0){
								$status ='<span class="badge badge-primary">' . getval("STR_ENTERED") . '</span>';
							}else if($row['status']  == 1){
								$status ='<span class="badge badge-success">' . getval("STR_APPROVED") . '</span>';
							}else {
								$status ='<span class="badge badge-danger">' . getval("STR_CANCELED") . '</span>';
							}

							$tpl->assign("STATUS", $status );

							$tpl->assign("CANCEL_REASON", $row['cancel_reason']);
			...
			// $today = date("d-m-Y");
			// $all_departments_access = db_get_array("SELECT indicator_category_id FROM new_module_permisions WHERE department_id IN (".$departments_array.") AND access_type = 1 AND active_date_from <= CAST(NOW() as datetime) AND active_date_to >= CAST(NOW() as datetime)", "indicator_category_id");
			// $all_departments_not_access = db_get_array("SELECT indicator_category_id FROM new_module_permisions WHERE department_id IN (".$departments_array.") AND access_type = 0 AND active_date_from <= CAST(NOW() as datetime) AND active_date_to >= CAST(NOW() as datetime)", "indicator_category_id");
