...
if (isset($_SESSION['id'])) {
	...
	$first_result = "SELECT * FROM module_notifications";
	...
	if (db_num_rows($result) > 0) {
		$tpl->CLEAR("NOTICE_ROWS");
		while ($row = db_fetch_array($result)) {
			$index++;
			$tpl->assign("ID", $index);
			$tpl->assign("NOTICE", $row['notice']);
			$tpl->assign("URL", $row['url']);
			$tpl->assign("FILE_URL", ' <a href="/'.$row['url'].'" class="btn btn-info btn-sm text-white"><i class="fa fa-link"></i> Ссылка</a>');
			...
			$tpl->parse("NOTICE_ROWS", "." . $moduleName . "row");
		}
	} else {
