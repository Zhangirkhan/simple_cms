<?php
$moduleName = "pps_mezhdunarodnaya_deyatelnost";
...
if (isset($_SESSION['id'])) {
	...
	$first_result = "SELECT * FROM module_table_names WHERE category_id = 2";
	...
	$ballses = db_get_data("SELECT sum(balls) as ballses FROM module_table_names WHERE category_id = 2 ", "ballses");
	$tpl->assign("CATEGORY_MAX_BALLS", $ballses + $all_category);
	$forms_count = db_get_data("SELECT count(id) as ids FROM module_table_names WHERE category_id = 2 ", "ids");
	$tpl->assign("FORMS_COUNT", $forms_count);
	
	$result = db_query("SELECT * FROM module_table_names WHERE category_id = 2  ORDER by table_name ASC");
	if (db_num_rows($result) > 0) {
		$indexa = 0;
		while ($row = db_fetch_array($result)) {
