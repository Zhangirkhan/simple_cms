...
if (isset($_POST['send'])) {
	...
	$result = db_query("SELECT * FROM new_module_users WHERE username = '" . $login . "' AND password = '" . $password . "'  LIMIT 1");
	...
	if (db_num_rows($result) > 0) {
		...
		if (isset($_POST['chk_save'])) {
			...
			setcookie("system_cookie", $cookie_value, time() + 60 * 60 * 24 * 30, "", $_SERVER['HTTP_HOST']);
		} else {
			if (isset($_COOKIE['system_cookie'])) {
				$cookie_value = "";
				setcookie("system_cookie", $cookie_value, 0, "", $_SERVER['HTTP_HOST']);
			}
		}
		...
		exit;
			...
			header("Location: https://" . $_SERVER['SERVER_NAME'] . "/" . LANG_INDEX . "/researcher_dashboard");
		exit;
		}else{
	...
	if (isset($_COOKIE['system_cookie'])) {
		$str = crypt_string($_COOKIE['system_cookie'], false);
