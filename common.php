...
define("SITE_URL", "https://" . getenv("HTTP_HOST") . $_SERVER['PHP_SELF']);
	...
	if (isset($_REQUEST[$value_name])) {
		switch ($type) {
			case 'int': {
					...
					if (is_numeric($result)) $result = abs(intval($result));
					else $result = 0;
					break;
				}
			case 'string': {
...
function assignList($assignTo, $valuesArray, $selectedValue = '', $fieldName = '')
{
	...
	if (is_array($valuesArray)) {
		...
		$option .= '<option value="0">Ничего не найдено</option>';
	}
...
{
	if (strpos($date, ":")) {
		preg_match("([0-9]{4})-([0-9]{1,2})-([0-9]{1,2}) ([0-9]{1,2}):([0-9]{1,2}):([0-9]{1,2})", $date, $regs);
...
{
	preg_match(" " . $var . "=([a-z0-9]+) ", $str, $regs);
...
function getImageTag($id, $align, $size = 1)
{
	...
	if (db_num_rows($result) > 0) {
		...
		if (strtolower($row['type']) == 'swf') {
			...
			if (empty($params)) $border = ' border="0"';
			else $border = '';
...
{
	if ($get_parent) $from_id = 'parent_id';
	else $from_id = 'id';
	if ($type != 'all') $where_type = " AND type != 'menu'";
	else $where_type = '';
...
{
	if ($parent_id > 0) $condition = ' AND id = ' . $parent_id;
	else $condition = '';
...
function getPageSort($page)
{
	...
	if ($sort['sortOrder'] == 0) $sort['sortOrder'] = "ASC";
...
function safe_string($text, $from_charset = 'UTF-8', $to_charset = 'CP1251')
{
	...
	preg_match("([0-9a-zA-Zà-ÿÀ-ß ]*)", $text, $regs);
...
function getFlatAccountNumber($city, $cityregion, $street, $house, $flat_number)
{

	return $city . $cityregion . $street . $house . $flat_number;
}


function getSelectOptionWithOthers($valuesArray, $selectedValue = "", $firstRowText = "", $fieldName = "")
{
	$option = null;
...
function checkAccessPage($page_link){
	if($page_link == '') return false;
	...
	$result = db_query("SELECT * FROM pages WHERE parent_id = ".$menu_id." AND mod_rewrite = '".$page_link."' AND visible = 1 AND deleted = 0 ORDER BY sortfield");
