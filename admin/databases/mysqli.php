<?php
	...
	function db_connect($db_host, $db_login, $db_password) {
		...
		if (!$mysql_link) {
			...
			db_query("SET SESSION time_zone = '+6:00'");
		}
	}
	...
	function db_query($query, $hide_errors = true) {
		// $filename = '/home/realkz/www/sql_query.txt';

		// $fd = fopen($filename, 'a+');
		// fwrite($fd, date("d.m.Y H:i:s")." : ".$query."\n");
		// fclose($fd);
...
/*
      ...
      $logline  = "\n[" . date( 'Y-m-d H:i:s' ) . '] ' . $stack[1]['file'];
      $logline .= "\nERROR = " . db_error();
      $logline .= "\nQUERY = " . var_export($query, true);
      file_put_contents( 'admin/upload/db_query_error.log', $logline, FILE_APPEND );
*/
      // error_log( 'db_query ERROR = ' . db_error() . '; QUERY = ' . substr($query, 0, 100) );
			return false;
		} else {
